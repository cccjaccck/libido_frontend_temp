...
export const HeaderWithSetting = withRouter(({ children, history, to }) => {
  const goBack = () => {
    history.go(-1);
  };
  ...
  return (
    <IosWrapper>
      ...
      <Container>
        ...
        />
        <Title>{children}</Title>
        <Link to={to} style={{ zIndex: "999" }}>
          <MdSettings size={24} color={"#4C5264"} />
        </Link>
      </Container>
    </IosWrapper>
  );
...
export const HeaderSearch = withRouter(
  ({
    children,
    placeholder,
    isClick,
    ...
    return (
      <>
        <Overlay isClick={isClick} onClick={onClickRemove} />
        <IosWrapper>
          <IosPadding />
          <Container>
            <UnderLine />
